...
%%
[A,B1,B,C1,C,D11,D12,D21,nx,nw,nu,nz,ny] = COMPleib('AC10');
...
%%
    ...
    for i=1:nx
        ...
        fill(tf,xf,'r');
        plot(T,y(:,i))
        xlabel(sprintf('$x_{%d}$',i),'interpreter','latex');
        ylabel(sprintf('$t$',i),'interpreter','latex');
    end
...
%%
    ...
    for idt=1:length(T)
        ...
        for i=1:length(iqc.Pi)
            ...
            for id_y=1:ny
                np = C(id_y,:)';
                xmax_t(id_y) =  sqrt(np'*Et^-1*np)+np'*xp;
                xmin_t(id_y) = -sqrt(np'*Et^-1*np)+np'*xp;
            end
            xmax = min([xmax,xmax_t],[],2);
            xmin = max([xmin,xmin_t],[],2);
        end
    ...
    for i=1:ny
        ...
        xlabel(sprintf('$t$',i),'interpreter','latex');
        set(gca,'TickLabelInterpreter','latex');
        set(gcf,'Position',[((i-1)*330) 502 330 250]);
        xlim([0,Tmax])
        grid on
        
        print(sprintf('img/ac10_y%d.eps',i),'-depsc2');
    end
end